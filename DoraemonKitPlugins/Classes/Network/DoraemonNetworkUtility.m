...
@implementation DoraemonNetworkUtility
...
- (__kindof NSArray *)_mapped:(id (^)(id, NSUInteger))mapFunc {
...
- (__kindof NSArray *)_flatmapped:(NSArray *(^)(id, NSUInteger))block {
...
- (NSArray *)_filtered:(BOOL (^)(id, NSUInteger))filterFunc {
    return [self _mapped:^id(id obj, NSUInteger idx) {
        return filterFunc(obj, idx) ? obj : nil;
    }];
}

- (void)_forEach:(void(^)(id, NSUInteger))block {
    [self enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        block(obj, idx);
    }];
}
...
NSString * const kDefaultsNetworkHostDenylistKey = @"com.network_host_denylist";

@implementation NSUserDefaults (Network)

- (NSArray<NSString *> *)_networkHostDenylist {
    return [NSArray arrayWithContentsOfFile:[
        self _defaultsPathForFile:kDefaultsNetworkHostDenylistKey
    ]] ?: @[];
}

- (void)set_networkHostDenylist:(NSArray<NSString *> *)denylist {
...
- (NSString *)_defaultsPathForFile:(NSString *)filename {
