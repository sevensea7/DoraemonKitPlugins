...
NSString *const kNetworkObserverEnabledStateChangedNotification = @"kNetworkObserverEnabledStateChangedNotification";
static NSString *const kNetworkObserverEnabledDefaultsKey = @"com.networkObserver.enableOnLaunch";
...
@implementation InternalRequestState
@end


@interface DoraemonNetworkObserver (NSURLConnectionHelpers)
...
@interface DoraemonNetworkObserver (NSURLSessionTaskHelpers)
...
@interface DoraemonNetworkObserver ()

@property (nonatomic) NSMutableDictionary<NSString *, InternalRequestState *> *requestStatesForRequestIDs;
@property (nonatomic) dispatch_queue_t queue;

@end

@implementation DoraemonNetworkObserver

#pragma mark - Public Methods

+ (void)setEnabled:(BOOL)enabled {
    BOOL previouslyEnabled = [self isEnabled];
    [NSUserDefaults.standardUserDefaults setBool:enabled forKey:kNetworkObserverEnabledDefaultsKey];
    ...
    if (previouslyEnabled != enabled) {
        [NSNotificationCenter.defaultCenter postNotificationName:kNetworkObserverEnabledStateChangedNotification object:self];
    }
}

+ (BOOL)isEnabled {
    return [[NSUserDefaults.standardUserDefaults objectForKey:kNetworkObserverEnabledDefaultsKey] boolValue];
}
...
+ (instancetype)sharedObserver {
    static DoraemonNetworkObserver *sharedObserver = nil;
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (numClasses > 0) {
            ...
            for (NSInteger classIndex = 0; classIndex < numClasses; ++classIndex) {
                ...
                if (class == [DoraemonNetworkObserver class]) {
                    continue;
                }
...
+ (void)injectIntoNSURLConnectionCancel {
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL swizzledSelector = [DoraemonNetworkUtility swizzledSelectorForSelector:selector];
        Method originalCancel = class_getInstanceMethod(class, selector);

        void (^swizzleBlock)(NSURLConnection *) = ^(NSURLConnection *slf) {
            [DoraemonNetworkObserver.sharedObserver connectionWillCancel:slf];
...
+ (void)injectIntoNSURLSessionTaskResume {
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (sel_af_resume) {
            NSMutableArray<Class> *classTree = GetAllSubclasses(baseResumeClass, NO).mutableCopy;
            for (NSInteger i = 0; i < classTree.count; i++) {
                [classTree addObjectsFromArray:GetAllSubclasses(classTree[i], NO)];
            }
...
+ (void)swizzleResumeSelector:(SEL)selector forClass:(Class)class {
    ...
    IMP implementation = imp_implementationWithBlock(^(NSURLSessionTask *slf) {
        ...
        [DoraemonNetworkObserver.sharedObserver URLSessionTaskWillResume:slf];
        ((void(*)(id, SEL))objc_msgSend)(
            slf, swizzledSelector
        );
    });
...
+ (void)injectIntoNSURLConnectionAsynchronousClassMethod {
    ...
    dispatch_once(&onceToken, ^{
        ...
        SendAsyncRequestBlock swizzleBlock = ^(Class slf,
            ...
            if (DoraemonNetworkObserver.isEnabled) {
                NSString *requestID = [self nextRequestID];
                [DoraemonNetworkRecorder.defaultRecorder
                ...
                [DoraemonNetworkRecorder.defaultRecorder recordMechanism:mechanism forRequestID:requestID];
        ...
        [DoraemonNetworkUtility replaceImplementationOfKnownSelector:selector
...
+ (void)injectIntoNSURLSessionAsyncDataAndDownloadTaskMethods {
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int selectorIndex = 0; selectorIndex < numSelectors; selectorIndex++) {
            ...
            if ([DoraemonNetworkUtility instanceRespondsButDoesNotImplementSelector:selector class:class]) {
            ...
            NSURLSessionNewTaskMethod swizzleBlock = ^NSURLSessionTask *(NSURLSession *slf,
                ...
                if (DoraemonNetworkObserver.isEnabled && completion) {
                    NSString *requestID = [self nextRequestID];
                    NSString *mechanism = [self mechanismFromClassMethod:selector onClass:class];
...
+ (void)injectWillSendRequestIntoDelegateClass:(Class)cls {
    ...
    WillSendRequestBlock undefinedBlock = ^NSURLRequest *(id slf,
                                                          ...
                                                          NSURLRequest *request,
                                                          NSURLResponse *response) {
        [DoraemonNetworkObserver.sharedObserver
    ...
    [DoraemonNetworkUtility replaceImplementationOfSelector:selector
...
+ (void)injectDidReceiveResponseIntoDelegateClass:(Class)cls {
    ...
    DidReceiveResponseBlock undefinedBlock = ^(id<NSURLConnectionDelegate> slf,
                                               NSURLConnection *connection,
                                               NSURLResponse *response) {
        [DoraemonNetworkObserver.sharedObserver connection:connection
...
+ (void)injectDidFinishLoadingIntoDelegateClass:(Class)cls {
    ...
    FinishLoadingBlock undefinedBlock = ^(id<NSURLConnectionDelegate> slf, NSURLConnection *connection) {
        [DoraemonNetworkObserver.sharedObserver connectionDidFinishLoading:connection delegate:slf];
    };
...
+ (void)injectTaskDidReceiveDataIntoDelegateClass:(Class)cls {
    ...
    DidReceiveDataBlock undefinedBlock = ^(id<NSURLSessionDataDelegate> slf,
                                           ...
                                           NSURLSessionDataTask *dataTask,
                                           NSData *data) {
        [DoraemonNetworkObserver.sharedObserver URLSession:session
...
+ (NSString *)requestIDForConnectionOrTask:(id)connectionOrTask {
    NSString *requestID = objc_getAssociatedObject(connectionOrTask, kRequestIDKey);
    if (!requestID) {
...
+ (void)setRequestID:(NSString *)requestID forConnectionOrTask:(id)connectionOrTask {
    objc_setAssociatedObject(
        connectionOrTask, kRequestIDKey, requestID, OBJC_ASSOCIATION_RETAIN_NONATOMIC
    );
}
...
- (id)init {
    ...
    if (self) {
        ...
        self.queue = dispatch_queue_create(
            "com.network.observer", DISPATCH_QUEUE_SERIAL
        );
    }
...
- (InternalRequestState *)requestStateForRequestID:(NSString *)requestID {
    InternalRequestState *requestState = self.requestStatesForRequestIDs[requestID];
    if (!requestState) {
        requestState = [InternalRequestState new];
...
@implementation DoraemonNetworkObserver (NSURLConnectionHelpers)
...
@implementation DoraemonNetworkObserver (NSURLSessionTaskHelpers)
