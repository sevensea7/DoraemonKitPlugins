...
@interface WMNetworkWebViewController () <WKNavigationDelegate>
...
@implementation WMNetworkWebViewController
...
- (id)initWithText:(NSString *)text {
    ...
    if (self) {
        ...
        NSString *htmlString = [NSString stringWithFormat:@"<head><meta name='viewport' content='initial-scale=1,maximum-scale=1,minimum-scale=1,user-scalable=no'></head><style type='text/css'>pre{white-space: pre-wrap; word-wrap: break-word;}</style><body><pre>%@</pre></body>", [WMNetworkUtility stringByEscapingHTMLEntitiesInString:text]];
...
- (void)viewDidLoad {
    ...
    if (self.originalText.length > 0) {
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"复制" style:UIBarButtonItemStylePlain target:self action:@selector(copyButtonTapped:)];
    }
}
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    if (navigationAction.navigationType == WKNavigationTypeOther) {
        ...
        WMNetworkWebViewController *webVC = [[[self class] alloc] initWithURL:[request URL]];
