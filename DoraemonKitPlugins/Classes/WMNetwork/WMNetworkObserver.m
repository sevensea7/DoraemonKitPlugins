...
static NSString *const kNetworkObserverEnabledDefaultsKey = @"com.weimai.networkObserver.enableOnLaunch";
...
@interface WMInternalRequestState : NSObject
...
@implementation WMInternalRequestState
@end


@interface WMNetworkObserver (NSURLConnectionHelpers)
...
@interface WMNetworkObserver (NSURLSessionTaskHelpers)
...
@interface WMNetworkObserver ()

@property (nonatomic) NSMutableDictionary<NSString *, WMInternalRequestState *> *requestStatesForRequestIDs;
@property (nonatomic) dispatch_queue_t queue;

@end

@implementation WMNetworkObserver
...
+ (instancetype)sharedObserver {
    static WMNetworkObserver *sharedObserver = nil;
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (numClasses > 0) {
            ...
            for (NSInteger classIndex = 0; classIndex < numClasses; ++classIndex) {
                ...
                if (class == [WMNetworkObserver class]) {
                    continue;
                }
...
+ (void)injectIntoNSURLConnectionCancel {
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL swizzledSelector = [WMNetworkUtility swizzledSelectorForSelector:selector];
        Method originalCancel = class_getInstanceMethod(class, selector);

        void (^swizzleBlock)(NSURLConnection *) = ^(NSURLConnection *slf) {
            [WMNetworkObserver.sharedObserver connectionWillCancel:slf];
...
+ (void)injectIntoNSURLSessionTaskResume {
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (sel_af_resume) {
            NSMutableArray<Class> *classTree = WMGetAllSubclasses(baseResumeClass, NO).mutableCopy;
            for (NSInteger i = 0; i < classTree.count; i++) {
                [classTree addObjectsFromArray:WMGetAllSubclasses(classTree[i], NO)];
            }
...
+ (void)swizzleResumeSelector:(SEL)selector forClass:(Class)class {
    ...
    IMP implementation = imp_implementationWithBlock(^(NSURLSessionTask *slf) {
        ...
        [WMNetworkObserver.sharedObserver URLSessionTaskWillResume:slf];
        ((void(*)(id, SEL))objc_msgSend)(
            slf, swizzledSelector
        );
    });
...
+ (void)injectIntoNSURLConnectionAsynchronousClassMethod {
    ...
    dispatch_once(&onceToken, ^{
        ...
        SendAsyncRequestBlock swizzleBlock = ^(Class slf,
            ...
            if (WMNetworkObserver.isEnabled) {
                NSString *requestID = [self nextRequestID];
                [WMNetworkRecorder.defaultRecorder
                ...
                [WMNetworkRecorder.defaultRecorder recordMechanism:mechanism forRequestID:requestID];
        ...
        [WMNetworkUtility replaceImplementationOfKnownSelector:selector
...
+ (void)injectIntoNSURLSessionAsyncDataAndDownloadTaskMethods {
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int selectorIndex = 0; selectorIndex < numSelectors; selectorIndex++) {
            ...
            if ([WMNetworkUtility instanceRespondsButDoesNotImplementSelector:selector class:class]) {
            ...
            NSURLSessionNewTaskMethod swizzleBlock = ^NSURLSessionTask *(NSURLSession *slf,
                ...
                if (WMNetworkObserver.isEnabled && completion) {
                    NSString *requestID = [self nextRequestID];
                    NSString *mechanism = [self mechanismFromClassMethod:selector onClass:class];
...
+ (void)injectWillSendRequestIntoDelegateClass:(Class)cls {
    ...
    WillSendRequestBlock undefinedBlock = ^NSURLRequest *(id slf,
                                                          ...
                                                          NSURLRequest *request,
                                                          NSURLResponse *response) {
        [WMNetworkObserver.sharedObserver
    ...
    [WMNetworkUtility replaceImplementationOfSelector:selector
...
+ (void)injectDidReceiveResponseIntoDelegateClass:(Class)cls {
    ...
    DidReceiveResponseBlock undefinedBlock = ^(id<NSURLConnectionDelegate> slf,
                                               NSURLConnection *connection,
                                               NSURLResponse *response) {
        [WMNetworkObserver.sharedObserver connection:connection
...
+ (void)injectDidFinishLoadingIntoDelegateClass:(Class)cls {
    ...
    FinishLoadingBlock undefinedBlock = ^(id<NSURLConnectionDelegate> slf, NSURLConnection *connection) {
        [WMNetworkObserver.sharedObserver connectionDidFinishLoading:connection delegate:slf];
    };
...
+ (void)injectTaskDidReceiveDataIntoDelegateClass:(Class)cls {
    ...
    DidReceiveDataBlock undefinedBlock = ^(id<NSURLSessionDataDelegate> slf,
                                           ...
                                           NSURLSessionDataTask *dataTask,
                                           NSData *data) {
        [WMNetworkObserver.sharedObserver URLSession:session
...
- (id)init {
    ...
    if (self) {
        ...
        self.queue = dispatch_queue_create(
            "com.wm.WMNetworkObserver", DISPATCH_QUEUE_SERIAL
        );
    }
...
- (WMInternalRequestState *)requestStateForRequestID:(NSString *)requestID {
    WMInternalRequestState *requestState = self.requestStatesForRequestIDs[requestID];
    if (!requestState) {
        requestState = [WMInternalRequestState new];
...
@implementation WMNetworkObserver (NSURLConnectionHelpers)
...
- (void)connection:(NSURLConnection *)connection
  ...
  redirectResponse:(NSURLResponse *)response
    ...
    [self performBlock:^{
        ...
        WMInternalRequestState *requestState = [self requestStateForRequestID:requestID];
...
@implementation WMNetworkObserver (NSURLSessionTaskHelpers)
